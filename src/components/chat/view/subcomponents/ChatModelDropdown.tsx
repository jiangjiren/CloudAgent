      ...
      {open && menuPos
        ? createPortal(
            <div
              ...
              className="z-[200] w-64 overflow-y-auto rounded-xl border border-border/60 bg-popover p-1 text-popover-foreground shadow-lg"
            >
              {connectedGroups.map((group, idx) => (
                <div
                  key={group.id}
                  className={idx > 0 ? "mt-1 border-t border-border/40 pt-1" : ""}
                >
